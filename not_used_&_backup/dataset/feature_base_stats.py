...
def mean_hp_difference_start(dataset) -> pd.DataFrame:  # feature
    ...
    mean_hp_difference["mean_hp_difference"] = np.subtract.reduce(
        mean_hp_difference[["p1_mean_hp_start", "p2_mean_hp_start"]], axis=1
    )
...
def sum_hp_last(dataset):  # feature
    ...
    sum_hp_last = pd.DataFrame(
        {"p1_sum_hp_last": p1_sum_hp, "p2_sum_hp_last": p2_sum_hp}
    )
...
def mean_atk_start(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        # calculating and appending the mean
        p2_mean_atk.append(
            (np.sum(p2_team["base_atk"]) + mean_atk * (6 - p2_known)) / 6
        )

    mean_atk_start = pd.DataFrame(
        {"p1_mean_atk_start": p1_mean_atk, "p2_mean_atk_start": p2_mean_atk}
    )
    # calculating the difference between the two means
    mean_atk_start["mean_atk_start_difference"] = np.subtract.reduce(
        mean_atk_start[["p1_mean_atk_start", "p2_mean_atk_start"]], axis=1
    )
...
def mean_def_start(dataset) -> pd.DataFrame:  # feature
    ...
    mean_def_start = pd.DataFrame(
        {"p1_mean_def_start": p1_mean_def, "p2_mean_def_start": p2_mean_def}
    )
    # calculating the difference between the two means
    mean_def_start["mean_def_start_difference"] = np.subtract.reduce(
        mean_def_start[["p1_mean_def_start", "p2_mean_def_start"]], axis=1
    )
...
def mean_spa_start(dataset) -> pd.DataFrame:  # feature
    ...
    mean_spa_start = pd.DataFrame(
        {"p1_mean_spa_start": p1_mean_spa, "p2_mean_spa_start": p2_mean_spa}
    )
    # calculating the difference between the two means
    mean_spa_start["mean_spa_start_difference"] = np.subtract.reduce(
        mean_spa_start[["p1_mean_spa_start", "p2_mean_spa_start"]], axis=1
    )
...
def mean_spd_start(dataset) -> pd.DataFrame:  # feature
    ...
    mean_spd_start = pd.DataFrame(
        {"p1_mean_spd_start": p1_mean_spd, "p2_mean_spd_start": p2_mean_spd}
    )
    # calculating the difference between the two means
    mean_spd_start["mean_spd_start_difference"] = np.subtract.reduce(
        mean_spd_start[["p1_mean_spd_start", "p2_mean_spd_start"]], axis=1
    )
...
def mean_stats_start(dataset) -> pd.DataFrame:  # feature
    ...
    mean_stats = pd.DataFrame(
        {"p1_mean_stats_start": p1_mean_stats, "p2_mean_stats_start": p2_mean_stats}
    )
    # calculating the difference between the two means
    mean_stats["mean_stats_start_difference"] = np.subtract.reduce(
        mean_stats[["p1_mean_stats_start", "p2_mean_stats_start"]], axis=1
    )
    return mean_stats
...
def sum_stats_last_2(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        if len(p1_team) != 0:
            ...
            # calculating and appending the sum
            p1_sum_stats.append(
                np.sum(p1_team["total"]) + (mean_total * (6 - p1_known))
            )
        else:
        ...
        if len(p2_team) != 0:
            ...
            # calculating and appending the sum
            p2_sum_stats.append(
                np.sum(p2_team["total"]) + (mean_total * (6 - p2_known))
            )
        else:
...
def crit_aggressive(dataset) -> pd.DataFrame:
    ...
    p1_crit_aggr = (
        mean_crit_df["p1_mean_crit"] * p1_offensive_ratio_df["p1_offensive_ratio"]
    )
    p2_crit_aggr = (
        mean_crit_df["p2_mean_crit"] * p2_offensive_ratio_df["p2_offensive_ratio"]
    )
