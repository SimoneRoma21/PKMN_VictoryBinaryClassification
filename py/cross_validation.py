...
def main(train_out_path, test_out_path):
    ...
    param_grid = {
        "C": [
            ...
            100,
            1000,
        ],  # Regola la regolarizzazione di lambda
    ...
    for C in param_grid["C"]:
        for penalty in param_grid["penalty"]:
            for solver in param_grid["solver"]:
                ...
                print(
                    f"C={C}, penalty={penalty}: mean acc = {mean_acc:.4f}, mean f1 = {mean_f1:.4f}, mean prec = {mean_prec:.4f}, mean rec = {mean_rec:.4f}"
                )
    ...
    submission = pd.DataFrame(
        {"battle_id": test_df["battle_id"], "player_won": test_preds}
    )
