...
def p1_pokemon_reflect(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        )[["name", "status"]].rename(columns={"name": "pkmn_name"})
        ...
        if len(turns_p1_state) != 0 and len(turns_p1_moves) != 0:
            turns = pd.concat([turns_p1_state, turns_p1_moves], axis=1).drop_duplicates(
                subset="pkmn_name", keep="last"
            )
...
def p2_pokemon_reflect(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        if len(turns_p2_state) != 0 and len(turns_p2_moves) != 0:
            turns = pd.concat([turns_p2_state, turns_p2_moves], axis=1).drop_duplicates(
                subset="pkmn_name", keep="last"
            )
            turns = turns[turns["name"] == "reflect"]
            turns = turns[turns["status"] != "fnt"]
...
def p1_pokemon_explosion(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        if len(turns_p1_state) != 0 and len(turns_p1_moves) != 0:
            ...
            turns = pd.concat(
                [
                    turns[turns["name"] == "explosion"],
                    turns[turns["name"] == "selfdestruct"],
                ],
                axis=0,
            )
...
def p1_reflect_ratio(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        p1_timeline = pd.DataFrame(
            [turn["p1_pokemon_state"] for turn in game["battle_timeline"]]
        )
        # counting turns of reflect
        p1_timeline["n_reflects"] = p1_timeline["effects"].apply(
            lambda x: x[0].count("reflect")
        )
        # appending ratio
...
def p2_reflect_ratio(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        p2_timeline = pd.DataFrame(
            [turn["p2_pokemon_state"] for turn in game["battle_timeline"]]
        )
        # counting turns of reflect
        p2_timeline["n_reflects"] = p2_timeline["effects"].apply(
            lambda x: x[0].count("reflect")
        )
        # appending ratio
...
def p1_lightscreen_ratio(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        p1_timeline["n_lightscreens"] = p1_timeline["effects"].apply(
            lambda x: x[0].count("lightscreen")
        )
        # appending ratio
...
def p2_lightscreen_ratio(dataset) -> pd.DataFrame:  # feature
    ...
    for game in dataset:
        ...
        # counting turns of lightscreen
        p2_timeline["n_lightscreens"] = p2_timeline["effects"].apply(
            lambda x: x[0].count("lightscreen")
        )
        # appending ratio
